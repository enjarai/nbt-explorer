...
impl Tag {
    ...
    fn read_type<T: Read>(data_type: u8, buf: &mut T) -> Result<Tag, TagError> {
        match data_type {
            // Byte (1 byte)
            0x01 => {
                Ok(Tag::ByteTag(buf.read_i8()?))
            }
            // Short (2 bytes)
            0x02 => {
                Ok(Tag::ShortTag(buf.read_i16::<BigEndian>()?))
            }
            // Int (4 bytes)
            0x03 => {
                Ok(Tag::IntTag(buf.read_i32::<BigEndian>()?))
            }
            // Long (8 bytes)
            0x04 => {
                Ok(Tag::LongTag(buf.read_i64::<BigEndian>()?))
            }
            // Float (4 bytes)
            0x05 => {
                Ok(Tag::FloatTag(buf.read_f32::<BigEndian>()?))
            }
            // Double (8 bytes)
            0x06 => {
                Ok(Tag::DoubleTag(buf.read_f64::<BigEndian>()?))
            }
            ...
            0x08 => {
                ...
                Ok(Tag::StringTag(String::from_utf8_lossy(&eat(buf, length as usize)?).parse().unwrap()))
            }
